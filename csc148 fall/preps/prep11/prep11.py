...
def mergesort3(lst: List) -> List:
    ...
    if len(lst) < 2:
        ...
        mid_sorted = mergesort3(lst[one_third:-one_third])
        right_sorted = mergesort3(lst[-one_third:])
...
def merge3(lst1: List, lst2: List, lst3: List) -> List:
    ...
    while index1 < len(lst1) and \
        ...
        if min([lst1[index1], lst2[index2], lst3[index3]]) == lst1[index1]:
            ...
            index3 += 1
    if index1 == len(lst1):
        return merged + helper(lst2[index2:], lst3[index3:])
    elif index2 == len(lst2):
        return merged + helper(lst1[index1:], lst3[index3:])
    else:
        return merged + helper(lst1[index1:], lst2[index2:])


def helper(lst1: list, lst2: list) -> list:
    """Return a sorted list with the elements in <lst1> and <lst2>.
...
def kth_smallest(lst: List, k: int) -> Any:
    ...
    if k < 0 or k >= len(lst):
        raise IndexError
    pivot = lst[0]
    smaller, larger = _partition(lst[1:], pivot)
    if k == 0 and len(smaller) == 1:
        return smaller[0]
    if k == 0 and len(smaller) == 0:
        return pivot
    if len(smaller) == 0 and k == 1:
        return kth_smallest(larger, 0)
    if len(smaller) < k:
