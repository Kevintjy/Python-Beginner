...
class Tree:
    ...
    def leaves(self) -> list:
        ...
        if self._subtrees == []:
            return [self._root]
        return sum([i.leaves() for i in self._subtrees], [])
    ...
    def branching_factor(self) -> float:
        ...
        if self.is_empty() or self._subtrees == []:
            return 0.0
        result = len(self) - len(self.leaves())
        return self._branching_factor_helper() / result
    ...
    def _branching_factor_helper(self) -> int:
        ...
        if self.is_empty() or self._subtrees == []:
            return 0
        temp = len(self._subtrees)
        for i in self._subtrees:
            temp += i._branching_factor_helper()
        return temp
    ...
    def insert(self, item: object) -> None:
        ...
        if self.is_empty():
            self._root = item
        if self._subtrees == []:
            self._subtrees.append(item)
        else:
            flag = random.randint(1, 3)
            if flag == 3:
                self._subtrees.append(item)
            else:
                new_flag = random.randint(1, len(self._subtrees))
                while type(self._subtrees[new_flag-1]) != Tree:
                    new_flag = random.randint(1, len(self._subtrees))
                self._subtrees[new_flag-1].insert(item)
